    ...
    if(error.length > 0 ){
        ...
        User.find({emailId:req.body.email, password:req.body.password}).exec((err,data)=>{
            if(err){
                ...
                */
                if(userDetails.verificationStatus == 'pending'){
                    ...
                    if(err){
                       ...
                       globalMethods.registrationEmail(token, req.body.email, (err, resp)=>{
                            if(err){
                                ...
                                res.send({status:1, message: 'User register successfully', data:'Please check your email'});
                            }
                       });
                    }
                });
    ...
    if(error.length>0){
        ...
        User.find({emailId:req.body.email}).exec((err,data)=>{
            if(err){
                ...
                data = data[0];
                payload = {'email':data.emailId, 'username':data.username}
                var token = globalMethods.getToken(payload);
...
exports.changePassword = (req,res)=>{
    ...
    if(error.length>0){
        ...
        User.findOneAndUpdate({emailId:req.body.email, password: req.body.oldPassword}, {$set:{password:req.body.newPassword}}).exec((err, data)=>{
            if(err){
                res.send({status:0, message:err});
            }else{
                res.send({status:1, message: 'Your password change successfully'});
            }
        });
...
exports.editProfile = (req,res)=>{
    // var photo = req.file.filename;
    var params = ['firstname','lastname','mobile','photo'];
    var error = globalMethods.checkRequireParam(params, req);
    if(error.length>0){
        res.send({status:0, message:error});      
    }else{
    
        User.findOneAndUpdate({emailId: req.body.email},{ $set: {firstname:req.body.firstname,lastname:req.body.lastname,mobile:req.body.mobile,photo:req.body.photo}}).exec((err, userDetail)=>{
            if(err){
                res.send({status:0, message:err});      
            }else{
                res.send({status:1, message:userDetail});      
            }
        });
...
exports.mailVerification1 = (req,res)=>{
    ...
    jwt.verify(req.query.token, config.securityToken, (err, decoded)=>{
        if(err){
            ...
            console.log(email);
            User.findOneAndUpdate({emailId:email, verificationStatus:'pending'}, { $set: { verificationStatus: 'active' }}).exec((err, userDetail)=>{
                if(err){
                    console.log(424);
                    res.send({status:0, message:err});
                }else{
                    res.send({status:1, message:'mail verified successfully'});
