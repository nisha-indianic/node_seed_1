    ...
    if(error.length>0){
        ...
        Admin.find({emailId:req.body.email}).exec((err, adminDetail)=>{
            if(err){
                ...
                adminDetail = adminDetail[0];
                payload = {'email':adminDetail.email, 'username':adminDetail.username}
                var token = globalMethods.getToken(payload);
...
exports.changePassword = (req,res)=>{
    ...
    if(error.length>0){
        ...
        Admin.findOneAndUpdate({emailId:req.body.email, password: req.body.oldPassword}, {$set:{password:req.body.newPassword}}).exec((err, data)=>{
            if(err){
                res.send({status:0, message:err});
            }else{
                res.send({status:0, message:'Your password change successfully'});
            }
        });
    }
}
        ...
        if(req.body.password == req.body.confirmPassword){
            ...
            jwt.verify(token, config.securityToken, (err,decoded)=>{
                if(err){
                    ...
                    Admin.findOneAndUpdate({emailId:data.email},{$set:{password:req.body.password}}).exec((err, adminDetail)=>{
                        if(err){
